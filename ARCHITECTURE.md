...
## Report agent (auto-generated CSRD PDF)

The pitch: bunq generates the CSRD source artefact on the company's behalf. The sustainability team downloads the PDF — no spreadsheets, no consultant assembly.

```
close.completed (audit row)
        │
        ▼
briefing.snapshot               (existing — buildBriefing JSON into the chain)
        │
        ▼
runReportAgent({orgId, label})  lib/agent/report-agent.ts (facade)
        │
        ▼
writeMonthlyReport()            lib/reports/auto-export.ts
        │
        ├─ buildBriefing(skipNarrative)            lib/reports/briefing.ts
        ├─ briefingDocument()                      lib/reports/render-briefing.tsx
        ├─ renderToBuffer()                        @react-pdf/renderer
        ├─ fs.writeFile data/exports/...pdf
        ├─ sha256 over the buffer
        └─ appendAudit type=bunq.report.generated  payload: relPath, bytes, sha256, period
        │
        ▼
GET /api/reports/download?path=data/exports/...   guarded streamer (data/exports/ only)
        │
        ▼
Dashboard "Reports" panel        app/page.tsx — last 6 by audit row
```

December closes additionally fire `writeAnnualReport` once per year (idempotent via audit-chain scan: `kind=annual, period.label=YYYY` rows are dedupe keys). The annual reuses `lib/reports/annual.ts` + `lib/agent/annual-narrative.ts` + `lib/reports/render-annual.tsx`.

Behaviour gated by `CARBO_AUTOEXPORT` (default `true`). Failure is non-fatal: a `bunq.report.failed` audit row captures the error and the close stays `COMPLETED`. The on-demand routes (`/briefing/pdf?...`, `/report/annual/[year]/pdf`) remain available as a manual fallback.

The visual style follows DESIGN.md (bunq Easy Green palette, Montserrat + Inter, Carbo brand mark + "Generated by bunq Business" caption). Cover page added in PR #6 — period as headline, three hero KPI cards, signature line for the sustainability lead, "CSRD ESRS E1-7 ready" pill. Mock the PDF without a DB via `npm run reports:mock` → `data/exports/mock-bunq-carbo-monthly.pdf`.
...
| Execution | `lib/bunq/payments.ts`, `lib/bunq/accounts.ts`, `lib/bunq/client.ts`, `lib/bunq/notes.ts`, `lib/bunq/draft-payment.ts` |
| Reporting | `app/report/[month]/page.tsx`, `lib/agent/narrative.ts`, `lib/agent/report-agent.ts`, `lib/reports/auto-export.ts`, `lib/reports/render-briefing.tsx`, `lib/reports/render-annual.tsx`, `app/api/reports/download/route.ts` |
