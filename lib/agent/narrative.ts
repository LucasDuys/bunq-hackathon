...
export const generateCsrdNarrative = async (params: {
  ...
  if (isAnthropicMock() || !process.env.ANTHROPIC_API_KEY) {
    ...
    return [
      ...
      `A Carbon Reserve of €${reserveEur.toFixed(2)} was allocated, funding ${creditTonnes.toFixed(2)} tCO₂e of carbon credits (${euPct.toFixed(0)}% EU-based, removal-weighted).`,
