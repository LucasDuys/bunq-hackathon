# SOUL — Carbo's soul (Claude Code edition)

> What makes Carbo cool, where the receipts live, and how the project graded itself.
> Read this first; everything else is detail. Mirrors `.openclaw/SOUL.md` for parallel structure.
> Claude Code's auto-loaded project context is `../CLAUDE.md` — read both.
...
Authority discipline: judges *validate*, code overrides on hard rules, money math stays deterministic. Documented in `../CLAUDE.md` §"Hard rules" and reflected in `lib/agents/dag/greenJudge.ts:268-286` (the override block that rejects zero-source recommendations regardless of Sonnet's verdict).
...
1. **`../CLAUDE.md`** — auto-loaded by Claude Code on every turn; the canonical procedure document.
2. **`../README.md`** — product pitch, dev quickstart, demo flow, "What's novel" table.
3. **`../PITCH.md`** — the 60-second pitch + "What this saves".
4. **`../ARCHITECTURE.md`** — system diagram, data model, state machine invariants, **Report agent** section.
5. **`../docs/agents/00-overview.md`** — the 8-agent DAG with per-agent links.
6. **`../research/INDEX.md`** — 14 numbered domain briefs (each ends in "Decisions for this build").
7. **`../JUDGE.md`** — the criterion → evidence → verify map.
8. **`../PROGRESS.md`** — done log; **`../todo.md`** — open work.

## Claude Code specifics

- **`../CLAUDE.md` is auto-loaded** on every turn. This file is the highlights overlay.
- **`.claude/settings.local.json`** holds per-session permissions. Don't commit secrets there.
- **Skills.** Use the `Skill` tool only for skills listed in the available-skills system reminder. Don't guess.
- **Hooks.** Configurable in `settings.json` under `hooks.*`. Treat hook output as user input.
- **Memory.** Persistent file-based memory at `~/.claude/projects/.../memory/`. Build understanding of who the user is over time; don't write memories for things derivable from `git log` or `CLAUDE.md`.
- **Plan mode.** Use `EnterPlanMode` for non-trivial tasks; user reviews + approves before any edits.
- **For UI / frontend work, invoke the `frontend-design` and `brand-guidelines` skills before writing code.** Standing user preference.

## What you must not miss

- **Mock defaults are safe.** `BUNQ_MOCK=1`, `ANTHROPIC_MOCK=1`, `DRY_RUN=1`, `CARBO_AUTOEXPORT=1`. Demo runs without API keys.
...
- [Anthropic Claude Code docs](https://docs.claude.com/en/docs/claude-code) — CLAUDE.md auto-loading, skills, hooks
...
- [OpenClaw workspace files explained (Capodieci, 2026)](https://capodieci.medium.com/ai-agents-003-openclaw-workspace-files-explained-soul-md-agents-md-heartbeat-md-and-more-5bdfbee4827a) — same files, OpenClaw convention
