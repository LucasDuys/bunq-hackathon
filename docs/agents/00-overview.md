...
- **Close machine:** `POST /api/close/run` → `lib/agent/close.ts` (12-state). Calls `runDag()` at the `DAG_RUNNING` state (commit 187aace) and persists the resulting `runId` on `close_runs.dag_run_id` for full agent-trace replay.
