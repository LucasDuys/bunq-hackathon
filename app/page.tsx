...
export default async function Overview() {
  ...
  const generatedReports = getGeneratedReports(DEFAULT_ORG_ID, 6);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Reports — auto-generated by bunq, ready to file under CSRD ESRS E1-7. */}
      <Card>
        <CardHeader>
          <div>
            <CodeLabel className="block mb-2">Generated by bunq Business · CSRD ESRS E1-7 ready</CodeLabel>
            <CardTitle>Reports</CardTitle>
          </div>
          <Badge tone="default">{generatedReports.length} on file</Badge>
        </CardHeader>
        <CardBody>
          {generatedReports.length === 0 ? (
            <div className="text-sm" style={{ color: "var(--fg-secondary)" }}>
              No reports yet — they auto-generate when a close completes.
            </div>
          ) : (
            <ul className="flex flex-col gap-2">
              {generatedReports.map((r) => {
                const title = r.kind === "annual"
                  ? `Annual · ${r.period.label}`
                  : `Monthly · ${r.period.label}`;
                const sizeKb = (r.bytes / 1024).toFixed(0);
                return (
                  <li
                    key={r.relPath}
                    className="ca-card flex items-center justify-between gap-4 px-4 py-2.5"
                    style={{ borderColor: "var(--brand-green-border)" }}
                  >
                    <div className="flex flex-col gap-0.5">
                      <span className="text-sm" style={{ color: "var(--fg-primary)" }}>{title}</span>
                      <span className="text-[11px] font-mono" style={{ color: "var(--fg-secondary)" }}>
                        {r.sha256.slice(0, 8)} · {sizeKb} KB
                      </span>
                    </div>
                    <a
                      href={`/api/reports/download?path=${encodeURIComponent(r.relPath)}`}
                      download
                      className="text-sm"
