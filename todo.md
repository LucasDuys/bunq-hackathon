...
- [ ] **Design tokens migration mostly done.** `/impacts`, `components/ImpactMatrix.tsx`, `app/invoices/page.tsx`, `SwitchCard.tsx`, `ClusterConstellation.tsx`, and `app/invoices/[id]/page.tsx` now use canonical tokens. Remaining: some `components/ui.tsx` Tailwind raw color classes + a few close/dashboard remnants. `TrendChart.tsx` keeps raw hex for SVG gradient `stopColor` (Recharts limitation). Low priority — CSS aliases keep them working.
